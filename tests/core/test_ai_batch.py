"""Tests for app.core.ai_batch.run_ai_batch_async."""
...
class TestRunAiBatchAsync:
    ...
    async def test_error_card_is_skipped(self):
        ...
        with patch("app.core.ai_batch.analyze_card_with_ai_async") as mock_ai:
            await run_ai_batch_async([card], on_progress, on_complete)
    ...
    async def test_successful_analysis_updates_card(self):
        ...
        with (
            ...
            patch("app.core.ai_batch.analyze_card_with_ai_async", AsyncMock(return_value=mock_result)),
            patch("app.core.ai_batch.save_raw_ai") as mock_save,
            patch("app.core.ai_batch.reprocess_candidates_from_raw") as mock_reprocess,
            patch("app.core.ai_batch.load_card_state_from_db") as mock_load,
        ):
            await run_ai_batch_async([card], on_progress, on_complete)
    ...
    async def test_skips_card_with_existing_ai_candidates(self):
        ...
        with (
            patch("app.core.ai_batch.get_card_state", return_value=existing_state),
            patch("app.core.ai_batch.analyze_card_with_ai_async") as mock_ai,
            patch("app.core.ai_batch.load_card_state_from_db") as mock_load,
        ):
    ...
    async def test_auth_error_aborts_remaining_cards(self):
        ...
        with (
            patch("app.core.ai_batch.get_card_state", return_value=None),
            patch("app.core.ai_batch.analyze_card_with_ai_async", AsyncMock(side_effect=auth_err)),
        ):
    ...
    async def test_progress_reported_for_multiple_cards(self):
        ...
        with (
            patch("app.core.ai_batch.get_card_state", return_value=None),
            patch("app.core.ai_batch.analyze_card_with_ai_async", AsyncMock(return_value=AIResult(best_name="X"))),
            patch("app.core.ai_batch.save_raw_ai"),
            patch("app.core.ai_batch.reprocess_candidates_from_raw"),
            patch("app.core.ai_batch.load_card_state_from_db"),
        ):
