...
class TestProcessFiles:
    ...
    def test_empty_list_calls_complete(self) -> None:
        """Empty file list calls on_complete immediately without creating a pool."""
        store = CardStore()
        service = ProcessingService(store)
        on_complete = MagicMock()

        with patch("app.core.services.processing_service.ProcessPoolExecutor") as mock_pool_cls:
            service.process_files([], on_complete=on_complete)
