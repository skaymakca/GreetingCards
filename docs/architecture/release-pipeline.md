...
The release pipeline is driven by `release-local.sh` (generated by `make configure-release`). Individual steps can be run selectively:

```
./release-local.sh build-draft       # full pipeline
./release-local.sh dmg               # single step
./release-local.sh build-notarize    # range
...
notarize  → scripts.notarize         (notarytool submit → staple)
...
Help output displays a formatted table with ID, Name, and Scope columns.
...
When pipeline steps are added, removed, or reordered, update the `STEPS` tuple of `Step` dataclass instances in `scripts/configure_release/generator.py`. Each `Step` has a `number`, `word_id` (lowercase alpha only, used as CLI argument), `label`, `command`, and `Scope`. This is the single source of truth for step definitions — the script body, help table, and word-to-number mapping are all generated from it.
