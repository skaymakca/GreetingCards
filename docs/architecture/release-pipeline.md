...
```
./release-local.sh 1-7           # full pipeline
./release-local.sh 3             # single step
./release-local.sh 3-7           # range

Step 1: make app                 (icon → content → tessdata → PyInstaller)
Step 2: scripts.sign             (inside-out codesign with $CODESIGN_IDENTITY)
Step 3: scripts.dmg              (dmgbuild → dist/Greeting-Cards-X.Y.Z.dmg)
Step 4: scripts.notarize         (notarytool submit → staple)
Step 5: scripts.release checksum (SHA256 → dist/Greeting-Cards-X.Y.Z.sha256)
Step 6: scripts.release changelog(extract → _build/release/release-notes.md)
Step 7: scripts.release draft    (gh release create --draft)
```
...
| Notarization "invalid signature"            | Binary not signed or UPX compressed          | Check `upx=False` in spec; re-run `./release-local.sh 2`         |
